...
class RouterService {
  async getAllRoutes(routesPath: string) {
    ...
    for (let route of routes) {
      ...
      let importedRouter: {default: Router} = await import(
        path.join(routesPath, route)
      );
      ...
      else router.use(route, importedRouter.default);
    }
    return router;
  }
}
